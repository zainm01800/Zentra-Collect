...
export default function InvoiceError({
  ...
  useEffect(() => { console.error("[invoices/[id]] boundary caught:", error); }, [error]);
    ...
    <div className="flex flex-col items-center justify-center gap-4 rounded-2xl p-14 text-center"
      ...
      <div>
        ...
        <p className="mt-1 text-[13px]" style={{ color: "var(--zn-ink-3)" }}>
          This invoice may have been removed or there was a loading error.
        </p>
      </div>
      <div className="flex gap-2">
        <button onClick={reset} className="zn-chip text-[13px]">Try again</button>
        <Link href="/chase-today" className="zn-pill text-[13px]">Back to Chase plan</Link>
      </div>
    </div>
  );
}

...
async function getInvoiceById(id: string) {
      ...
      if (user) {
        ...
        if (member) {
          const { data: row } = await supabase
            ...
            .select(`*, zentra_customers(name, email, contact_role, relationship_type)`)
            .eq("id", id)
            .eq("account_id", member.account_id)
            .maybeSingle();

          if (row) {
            const customer = row.zentra_customers as { name: string; email?: string; contact_role?: string; relationship_type?: string } | null;
            ...
            return {
              ...
              amountOutstanding: parseFloat(row.amount_outstanding ?? row.amount),
              currency: row.currency as "GBP",
              status: fmtStatus(row.status ?? ""),
              daysOverdue,
              chaseCount: row.previous_chase_count ?? 0,
              lastChasedAt: row.last_chased_date ?? null,
              promisedPaymentDate: row.promised_payment_date ?? null,
              relationshipType: customer?.relationship_type ?? "regular customer",
              notes: "",
              paymentLink: "",
              lineItems: (row.line_items ?? []) as { description: string; quantity: number; unitPrice: number }[],
              activityHistory: [] as { id: string; type: string; title: string; description: string; createdAt: string }[],
              isDemo: false,
            };
          }
        }
  ...
  const demo = demoInvoices.find((item) => item.id === id);
  if (!demo) return null;
...
export default async function InvoicePage({
  ...
  const urgency = getInvoiceUrgency(invoice as Parameters<typeof getInvoiceUrgency>[0]);
  const action  = getSuggestedAction(invoice as Parameters<typeof getSuggestedAction>[0]);
        ...
        <div className="zn-card px-5 py-4 flex flex-col gap-0">
          ...
          <div className="grid grid-cols-2 gap-y-0 divide-y" style={{ borderColor: "var(--zn-line-soft)" }}>
            {[
              ...
              { label: "Issue date",     value: formatDate((invoice as { issueDate?: string }).issueDate ?? invoice.dueDate) },

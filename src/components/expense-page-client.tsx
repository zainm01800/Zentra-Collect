...
export function ExpensePageClient() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── AI categorise banner — shown when there are any expenses ────── */}
      {entries.length > 0 && (
        <div
        ...
        >
          ...
          <p className="flex-1 text-[12.5px]" style={{ color: "var(--zn-info)" }}>
            {uncategorisedEntries.length > 0 ? (
              ...
              <>All expenses categorised — AI can re-verify or correct any category.</>
            )}
          </p>
          <div className="flex items-center gap-2 flex-shrink-0">
            {uncategorisedEntries.length > 0 && (
              <button
                ...
                disabled={classifying}
                className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-semibold transition-colors disabled:opacity-60"
                style={{ background: "var(--zn-info)", color: "#fff" }}
              >
                <Sparkles className="size-3" />
                {classifying ? "Classifying…" : `Classify ${uncategorisedEntries.length} uncategorised`}
              </button>
            )}
            <button
              type="button"
              onClick={() => handleReclassifyWithAI(entries)}
              disabled={classifying}
              className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-semibold transition-colors disabled:opacity-60"
              style={{
                background: uncategorisedEntries.length > 0 ? "transparent" : "var(--zn-info)",
                color: uncategorisedEntries.length > 0 ? "var(--zn-info)" : "#fff",
                border: uncategorisedEntries.length > 0 ? "1px solid color-mix(in srgb, var(--zn-info) 40%, transparent)" : "none",
              }}
            >
              <Sparkles className="size-3" />
              {classifying ? "Classifying…" : `Re-classify all ${entries.length}`}
            </button>
          </div>
        </div>
      )}
